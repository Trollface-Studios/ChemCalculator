...
public class Core {
	...
	public static void main(String[] args) {
		...
		Base.log("welcome, select your element.");
		...
		for (int j = 0; j < 47; j++) {
			if (listE[j].name.equals(n)) {
				Base.log("VICTORY");
				found = true;
				Base.log("element:  " + listE[j].name);
				Base.log("molarmass: " + listE[j].molarmass);
				if (listE[j].electronegativity != 0) {
					Base.log("electronegativity: "
							+ listE[j].electronegativity);
				} else {
					Base.log("electronegativity: "
							+ listE[j].electronegativity1);
				}
				Base.log("density: " + listE[j].density);
				Base.log("number of protons: " + j);
			}

		}

		if (!found)
			Base.log("SUP, you entered a wrong parameter");
		Base.log("Dumb fuck.");
	}
}
